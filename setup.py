...
try:
    ...
    print >> sys.stderr, 'To build catfish you need https://launchpad.net/python-distutils-extra'
    sys.exit(1)
...
def move_desktop_file(root, target_data, prefix):
    ...
    old_desktop_path = os.path.normpath(root + target_data +
                                        '/share/applications')
    old_desktop_file = old_desktop_path + '/catfish.desktop'
    desktop_path = os.path.normpath(root + prefix + '/share/applications')
    desktop_file = desktop_path + '/catfish.desktop'
    ...
    if not os.path.exists(old_desktop_file):
        ...
        desktop_file = desktop_path + '/extras-catfish.desktop'
        try:
...
class InstallAndUpdateDataDirectory(DistUtilsExtra.auto.install_auto):
    def run(self):
        ...
        target_pkgdata = target_data + 'share/catfish/'
        target_scripts = '/' + os.path.relpath(self.install_scripts, self.root) + '/'

        values = {'__catfish_data_directory__': "'%s'" % (target_pkgdata),
